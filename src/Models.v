...
Section Models.
  ...
  Section SymbolicModel.
    ...
    (* Indistinguishability is defined on two sequences of terms that have the same type and are the same length*)
  ...
  Section CompInterp.
    ...
    (* Utility function *)
    Definition curry_rands T eta (c : rands eta -> arands eta -> Comp T) : tuple (Bvector eta) (rand_bound + arand_bound) -> Comp T.
      intros.
    ...
    Definition indist dom (l1 l2 : hlist CompDomain dom) : Prop :=
      forall (f : bool_comp), poly_time (f dom l1) 
                         -> poly_time (f dom l2)
