...
Lemma head_prop_sub A (P : A -> Prop) a a' l l' :
  ...
  - reflexivity.
  - exfalso.
    eapply H; [eapply sublist_in|]; eassumption.
Qed.
...
Lemma subs_length_sub A (l l1 l2 : list A) :
  ...
  induction l as [|x l]; intros l1 l2 L1 L2 e1 e2 H.
...
Lemma tail_unique_prop (A : Type) (P : A -> Prop) (a1 a2 : A)
  ...
  pose proof sub_cons S2 as S2'.
  destruct (subs_compare S1 S2) as [[e | s] | s]; [assumption | ..];
  exfalso;
...
Definition first_prop_dec (A : Type) (P : A -> Prop)
           (P_dec : forall a, {P a} + {~P a}) (l : list A) :
  {a : A & {l' : _ & {L : _ | l = L ++ (a :: l') /\
                              (forall a', In a' L -> ~P a') /\
                              P a}}} +
  {forall a, In a l -> ~P a}.
  induction l as [|x l].
  - right; auto.
  - destruct (P_dec x) as [y | n].
    + left.
      exists x.
      exists l.
      exists [].
      split; [|split]; auto.
    + destruct IHl as [[a' [l' [L' [eL [nL y]]]]] | n'].
      * left.
        exists a'.
        exists l'.
        exists (x :: L').
...
Lemma last_prop_dec (A : Type) (P : A -> Prop)
      (P_dec : forall a, {P a} + {~P a}) (l : list A) :
  {a : A & {l' : _ | In_with_tail a l' l /\
                     (forall a', In a' l' -> ~P a') /\
                     P a}} +
  {forall a, In a l -> ~P  a}.
  pose proof first_prop_dec P_dec (rev l) as F.
  destruct F as [[a [l' [L [eL [nL y]]]]] | n].
  - left.
    exists a.
    exists (rev L).
    split; [|split].
    + unfold In_with_tail.
      apply sub_app.
      exists (rev l').
      assert (rev (rev l) = rev (L ++ a :: l')) by (rewrite eL; reflexivity).
